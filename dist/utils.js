...
export function extractTemplate(content, key, which) {
    ...
    while (i < content.length) {
        if (content.slice(i, i + startTag.length) === startTag) {
            ...
            if (!prev.inTagAndNeedSkip)
                prev.text += currentNode.text;
            currentNode = prev;
            i += endTag.length;
        }
        else {
            if (!currentNode.inTagAndNeedSkip)
                currentNode.text += content[i];
            i++;
        }
    }
    return currentNode.text;
}
export function resolveTemplateByConfig(templateData, config) {
...
export function setupReactSwc(root, isTs) {
